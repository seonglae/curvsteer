...
def load_dictionary(cfg):
    ...
    sf = load_file(path)
    return (sf["W_enc"], sf["b_enc"], sf["W_dec"], sf["b_dec"], None)


def dictionary_arms(cfg, model, met, pos, neg, fit_idx, mean_g, d, ranks):
    W_enc, b_enc, W_dec, b_dec, thr = load_dictionary(cfg)
    F = W_enc.shape[1]
    acts, y = [], []
    for texts, lab in ((pos, 1.0), (neg, -1.0)):
        for i in fit_idx:
            ids, _ = met.encode([texts[i]])
            # No hook: the residual at the site is a returned value.
            act = np.asarray(model.residual_at_site(ids)[0][met.plen:],
                             dtype=np.float32)
